...
const Messages = ({loggedInUserId, loggedInFirstName}) => {
  ...
  const fetchUserMessages = async (userId) => {
    try {
      const response = await fetch(`https://localhost:8443/api/v1/messages/user/${userId}`);
      if (response.ok) {
    ...
    try {
      const response = await fetch(`hhttps://localhost:8443/api/v1/messages/users/${userId}`);
      if (response.ok) {
